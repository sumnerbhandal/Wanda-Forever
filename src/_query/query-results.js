      ...
      let tagInstance = props.activeTags[h];
         ...
         for (let i = 0; i < data.contracts.length; i++) {
          const Labels = data.contracts.[i].labels;
            ...
            if (Labels !== "") {
              let labelCheck = Labels.filter((record) => record.name === tagInstance);
    ...
    {console.log(labelMatch)}  */}
      <div className="results">
        ...
        <div className="top-results">
          <h3>{labelMatch.length} contract(s) matching {props.activeTags.length < 2 ? `"${props.activeTags}"` : "selected labels"}</h3>
            ...
            <div id={labelResult.id} className="live-result-container">
              ...
              <div className="contract-details">
                <div className="contract-name">
                  <p><strong>{labelResult.name}</strong></p>
                </div>
      ...
      <div className="contract-view"><EmploymentContract /></div>
    </>
  );
}
